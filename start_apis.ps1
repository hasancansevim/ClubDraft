...
Start-Process -NoNewWindow dotnet "run --no-build --project src/Services/Draft/API/ClubCraft.Draft.API.csproj --urls http://localhost:5001" -RedirectStandardOutput draft.log -RedirectStandardError draft_err.log
...
Start-Process -NoNewWindow dotnet "run --no-build --project src/Services/MatchEngine/API/ClubCraft.MatchEngine.API.csproj --urls http://localhost:5006" -RedirectStandardOutput matchengine.log -RedirectStandardError matchengine_err.log
